...
% CALLING LOCAL FUNCTIONS DEFINED AT BOTTOM
...
AWGN_Ch = comm.AWGNChannel('EbNo', 10, 'BitsPerSymbol', BitsPerSymbol);
...
% --- PLOTTING ---
figure('Name', 'QPSK Constellations', 'Color', 'white');
subplot(1,3,1); plot(real(Con_Ray), imag(Con_Ray), '.'); title('QPSK Rayleigh'); axis square; grid on;
subplot(1,3,2); plot(real(Con_Ric), imag(Con_Ric), '.'); title('QPSK Rician'); axis square; grid on;
subplot(1,3,3); plot(real(Con_AWGN), imag(Con_AWGN), '.'); title('QPSK AWGN'); axis square; grid on;

% --- 6. OUTPUT ---
audiowrite('Out_QPSK_Rayleigh.wav', Decoder_1(Bits_Ray', Levels), Fs);
audiowrite('Out_QPSK_Rician.wav', Decoder_1(Bits_Ric', Levels), Fs);
audiowrite('Out_QPSK_AWGN.wav', Decoder_1(Bits_AWGN', Levels), Fs);
disp('QPSK Audio files saved.');


%% --- LOCAL HELPER FUNCTIONS (PASTED BELOW) ---
